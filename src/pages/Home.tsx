...
const TABS: TabConfig[] = [
  { id: 'env', label: 'Env & VMs', icon: <Layers size={20} />, permission: 'env', adminOnly: false },
  { id: 'exec', label: 'Exec', icon: <Terminal size={20} />, permission: 'exec', adminOnly: false },
...
export default function Home() {
  ...
  const canAccessTab = useCallback((tab: TabConfig): boolean => {
    if (tab.adminOnly) return isAdmin;
    if (tab.permission) return hasPermission(tab.permission);
    return true;
  }, [isAdmin, hasPermission]);

  const mobileVisibleTabs = TABS.filter(t => canAccessTab(t));

  useEffect(() => {
    const currentTab = TABS.find(t => t.id === activeTab);
    if (currentTab && !canAccessTab(currentTab)) {
      const firstAvailable = TABS.find(t => canAccessTab(t));
    ...
    <div className="flex flex-col h-screen bg-black text-white overflow-hidden font-sans">
      ...
      <div className="h-14 border-b border-zinc-800 flex items-center px-3 md:px-4 bg-zinc-950 shrink-0">
        ...
        <div className="hidden md:flex items-center gap-2 ml-4 w-auto justify-end">
          {TABS.filter(t => !t.adminOnly && !t.mobileOnly && canAccessTab(t)).map(tab => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg transition-colors text-sm ${activeTab === tab.id ? 'bg-zinc-800 text-blue-400' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/50'}`}
            >
              {tab.icon} {tab.label}
            </button>
          ))}
      ...
      <div className="md:hidden flex items-center justify-around bg-zinc-950 border-t border-zinc-800 p-2 shrink-0 z-50">
        {mobileVisibleTabs.filter(t => !t.mobileOnly).map(tab => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
